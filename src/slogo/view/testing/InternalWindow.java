...
public class InternalWindow extends Region {
  ...
  public void setPosition(int rowIndex, int colIndex){
    setPosition(rowIndex, colIndex, 1, 1);
  }

  public void setPosition(int rowIndex, int colIndex, int rowSpan, int colSpan){
    double colSize = 120;
    double rowSize = 60;
    setLayoutX(colIndex * colSize);
    setLayoutY(rowIndex * rowSize);

    Region region = (Region) getChildren().get(0);
    region.setPrefSize(colSpan * colSize, rowSpan * rowSize);
  }
